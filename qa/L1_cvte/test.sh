...
pushd .
cd /workspace/nvidia-examples/cvte
...
NUM_PROCESSES=$NUM_GPUS EXPECTED_CER=${GOLD_CER["cvte"]} EXPECTED_PERF=${GOLD_PERF["${GPU_NAME}x${NUM_GPUS}_cvte"]} DATASET=/workspace/datasets/thchs30/small/ bash -e ./run_benchmark.sh
