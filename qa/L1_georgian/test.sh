...
pushd .
cd /workspace/nvidia-examples/georgian
...
NUM_PROCESSES=$NUM_GPUS EXPECTED_WER=${GOLD_WER["georgian"]} EXPECTED_PERF=${GOLD_PERF["${GPU_NAME}x${NUM_GPUS}_georgian"]} DATASET=/workspace/datasets/georgian/ bash -e ./run_benchmark.sh
