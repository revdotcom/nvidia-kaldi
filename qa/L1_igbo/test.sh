...
pushd .
cd /workspace/nvidia-examples/igbo
...
NUM_PROCESSES=$NUM_GPUS EXPECTED_WER=${GOLD_WER["igbo"]} EXPECTED_PERF=${GOLD_PERF["${GPU_NAME}x${NUM_GPUS}_igbo"]} DATASET=/workspace/datasets/igbo/ bash -e ./run_benchmark.sh
