...
pushd .
cd /workspace/nvidia-examples/multi-cn
...
NUM_PROCESSES=$NUM_GPUS EXPECTED_WER=${GOLD_WER["multi-cn"]} EXPECTED_PERF=${GOLD_PERF["${GPU_NAME}x${NUM_GPUS}_multi-cn"]} DATASET=/workspace/datasets/multi-cn/ bash -e ./run_benchmark.sh
