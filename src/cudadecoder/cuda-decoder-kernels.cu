...
/*
   This kernel preprocess the necessary information for expand (scan of the
   outgoing degrees)
   and explicitly prune the tokens

   The ExpandArc kernel writes the new raw token list in the aux_q. However, the
   cutoff
   was progressively lowered during the computation, and some tokens now have a
   cost > cutoff.
   During the contract stage of this kernel, we remove such tokens.
   We also remove duplicates, i.e. tokens pointing to the same state, but with
   token.cost > best_cost_for_that_state
   ...
   -> Otherwise, we will move it to the main_q. We also read its arc degree in
   the FST graph

   3) We move the non-pruned tokens into the main q. After a local prefix sum,
   we request a spot in the main_q for those tokens using the
   main_q_end_and_narcs counter.
   main_q_end_and_narcs.split.end contains the number of tokens in the main q
   until now
   main_q_end_and_narcs.split.narcs contains the number of arcs in the main q
   until now

   We also compute the degrees prefix sum in one pass using the
   main_q_end_and_narcs.split.narcs
 */

// Important : pass the struct PreprocessParams by copy - passing it using a ref
// will not work (CPU -> GPU)
__global__ void preprocess_and_contract_kernel(DeviceParams cst_dev_params,
  ...
  //
  // The reason why we store those two values together is because they are
  // linked (see below)
  //
  // (1) We need a spot to store those tokens in the main_q
  // We will ask the main_q counter where to store those tokens, the answer will
  // be
  // an offset of the main_q. We will store our tokens in positions :
  // d_main_q_state[sh_main_q_global_block_offset.ntokens],
  // d_main_q_state[sh_main_q_global_block_offset.ntokens+1]...
  //
  // (2) sh_main_q_global_block_offset.narcs contains the number of arcs in the
  // main_q up until index sh_main_q_global_block_offset.ntokens
  // ie the number of arcs going out of all states in
  // d_main_q_state[0..sh_main_q_global_block_offset.ntokens]
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    // The condition of the for loop is the same for all threads in the CUDA
    // block
    ...
    KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(block_offset, thread_idx,
      ...
      if (aux_q_idx < aux_q_end) {
        ...
        // We know we have a token associated with token_state in the queue with
        // the cost state_best_cost
        ...
        if (token_int_cost < int_cutoff) {
          // If generating lattices, keeping the token as long as its not too
          // bad compared to best path
          // to that state
          // If not generating lattices (one-best), keeping the token if its the
          // best for that state
      ...
      // local indexes of the survival tokens
      ...
      if (KALDI_CUDA_DECODER_IS_LAST_1D_THREAD()) {
        ...
        // this CTA
        // (we need space for token_and_arc_count_block_sum.split.ntokens
        // tokens)
        // informing the main_q that our survival tokens contain
        // token_arc_count_block_sum.split.narcs arcs
        //
        // We then store the return value, which is the global offset on where
        // to store those tokens,
      ...
      if (!is_pruned) {
        ...
        // = (narcs until now in the main queue) + (narcs until this thread in
        // the CTA)
...
/*
   ...
   The tokens are already in the main q (they were placed here by a previous
   "contract and preprocess").

   We cannot prune non-optimal tokens, because the tokens are already in the
   main_q (we cannot prune
   the main_q - it would break the prev_token indexes). To avoid doing
   unnecessary computation
   in the expand kernel, we simulate the pruning by setting non-optimal token's
   degree to 0
   We then rely on the 1 thread = 1 arc exact load balacing of expand to ignore
   that token

   Please note that even if 0 threads will perform work on an ignored token in
   expand (degree = 0),
   it is not exactly the same as pruning it : the main_q accesses will not be
   perfectly coalesced
   in expand, because some "dead" tokens exist between living ones

   For the preprocess stage we have to compute the prefix sum of the tokens arc
   degrees
   Here we have to do the prefix sum in two passes : first local prefix sums
   inside CUDA block,
   then in a second kernel (finalize_preprocess_in_place), we add the necessary
   block offsets to end up
   with the global prefix sum

   This preprocess step is used in ProcessEmitting. Tokens were placed in main_q
   by
   the ProcessNonEmitting of the previous frame. We cannot renumber them (it
   would break
   the prev_token index). We preprocess in place, leaving things as they are in
   main_q
...
__global__ void preprocess_in_place_kernel(DeviceParams cst_dev_params,
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(block_offset, thread_idx,
      ...
      // list them in a separate list. They are associated to the representative
      // for that state (cf below)
      ...
      if (main_q_idx < main_q_end) {
        ...
        // the cost state_best_cost
        int hash_idx;
        HashmapValueT h_val = hashmap_find(
            cst_dev_params.d_hashmap_values.lane(ilane), token_state,
            cst_dev_params.hashmap_capacity, &hash_idx);
        const IntegerCostType state_best_int_cost_argmin =
            h_val.min_and_argmin_int_cost
                .y;  // TODO for nonoptimal save that in nextstate
        bool is_representative = (state_best_int_cost_argmin == main_q_idx);
        cst_dev_params.d_main_q_representative_id.lane(ilane)[main_q_idx] =
            is_representative ? (-hash_idx - 1)  // -1 to force it < 0
                              : hash_idx;
        ...
        if (representing_state) {
          if (token_int_cost < int_cutoff) {
            ...
            cst_dev_params.d_main_q_arc_offsets.channel(ichannel)[main_q_idx] =
                start;  // TODO put offset in next_state
          }
          n_extra_prev_token =
    ...
    KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(block_offset, thread_idx,
      ...
      if (KALDI_CUDA_DECODER_IS_LAST_1D_THREAD()) {
        ...
        if (aux_q_index_block_offset + total_successors_in_block <
          ...
          // We do not jump to finalize_kernel now, because only threadIdx.x ==
          // 0
          // is executing this
...
__global__ void load_channels_state_in_lanes_kernel(DeviceParams cst_dev_params,
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    lane_counters->main_q_n_extra_prev_tokens =
        channel_counters->prev_main_q_n_extra_prev_tokens;
    CostType beam = channel_counters->prev_beam;  // TODO rename prev_beam is
                                                  // actually the new frame beam
    IntegerCostType int_beam = floatToOrderedInt(beam);
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    KALDI_CUDA_DECODER_1D_KERNEL_LOOP(main_q_idx, main_q_end) {
      ...
      const int2 local_sum_offset =
          cst_dev_params.d_main_q_block_sums_prefix_sum.lane(
              ilane)[local_sum_idx];
      int32 hash_idx = cst_dev_params.d_main_q_representative_id.lane(
          ilane)[main_q_idx];  // TODO rename
...
/*
   ...
   Used to avoid calling multiple "heavy lifting" kernels for the tail of non
emitting
...
Note : For a detailed description on how the Preprocess and Expand operation
work,
please refer to the PreprocessInPlace and ExpandArc kernel implemention. The
algorithm are
described there. In this kernel, we compute simplified version of preprocess and
expand, because
    ...
    while (main_q_narcs > 0) {
      ...
      KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(offset, thread_idx,
        ...
        if (main_q_arc_idx < main_q_narcs) {
          ...
          // const int32 prev_state =
          // cst_dev_params.d_main_q_state_and_cost.channel(ichannel)[main_q_idx].x;
        ...
        if (has_successor) {
          ...
          cst_dev_params.d_aux_q_acoustic_cost.lane(ilane)[aux_q_idx] =
              0.0f;  // TODO remove
                     // const int32 prev_state =
          // cst_dev_params.d_main_q_state_and_cost.channel(ichannel)[main_q_idx].x;
          // const CostType prev_cost  =
          // orderedIntToFloat(cst_dev_params.d_main_q_state_and_cost.channel(ichannel)[main_q_idx].y);
        }
      ...
      KALDI_CUDA_DECODER_1D_BLOCK_OFFSET_KERNEL_LOOP(offset, thread_idx,
        ...
        if (has_valid_nonpruned_token) {
          ...
          cst_dev_params.d_main_q_info.lane(ilane)[main_q_idx] =
              cst_dev_params.d_aux_q_info.lane(ilane)[aux_q_idx];
          cst_dev_params.d_main_q_acoustic_cost.lane(ilane)[main_q_idx] =
              cst_dev_params.d_aux_q_acoustic_cost.lane(
                  ilane)[aux_q_idx];  // TODO remove
        }
...
__global__ void get_best_cost_kernel_step2(DeviceParams cst_dev_params,
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    // Use final if we want to use final and if we found a final state in the
    // token list
...
__global__ void update_beam_using_histogram_kernel(DeviceParams cst_dev_params,
  ...
  KALDI_CUDA_DECODER_BATCH_KERNEL_LOOP(ilane, nlanes) {
    ...
    CostType beam = orderedIntToFloat(lane_counters->int_beam);
    CostType min_cost = orderedIntToFloat(
