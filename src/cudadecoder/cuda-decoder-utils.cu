// cudadecoder/cuda-decoder-utils.cu
//
...
// we need to have an appropriate initial capacity (is set using a parameter in
// CudaDecoderConfig)
InfoTokenVector::InfoTokenVector(int32 capacity, cudaStream_t copy_st)
    : capacity_(capacity), copy_st_(copy_st) {
  KALDI_VLOG(2) << "Allocating InfoTokenVector with capacity = " << capacity_
                << " tokens";
  KALDI_DECODER_CUDA_API_CHECK_ERROR(
...
InfoTokenVector::InfoTokenVector(const InfoTokenVector &other)
    : InfoTokenVector(other.capacity_, other.copy_st_) {}

void InfoTokenVector::Reset() { size_ = 0; };

void InfoTokenVector::CopyFromDevice(
    InfoToken *d_ptr, int32 count) {  // TODO add the Append keyword
  Reserve(size_ + count);             // making sure we have the space

  KALDI_DECODER_CUDA_API_CHECK_ERROR(
      cudaMemcpyAsync(&h_data_[size_], d_ptr, count * sizeof(*h_data_),
...
void InfoTokenVector::Clone(const InfoTokenVector &other) {
  ...
  KALDI_DECODER_CUDA_API_CHECK_ERROR(
      cudaMemcpyAsync(h_data_, h_data_other, size_ * sizeof(*h_data_),
                      cudaMemcpyHostToHost, copy_st_));
...
void InfoTokenVector::Reserve(int32 min_capacity) {
  ...
  while (capacity_ < min_capacity) capacity_ *= 2;

  KALDI_VLOG(2) << "Reallocating InfoTokenVector on host (new capacity = "
                << capacity_ << " tokens).";
  ...
  if (!h_data_) KALDI_ERR << "Host ran out of memory to store tokens. Exiting.";

  if (size_ > 0)
    KALDI_DECODER_CUDA_API_CHECK_ERROR(
        cudaMemcpyAsync(h_data_, h_old_data, size_ * sizeof(*h_data_),
                        cudaMemcpyHostToHost, copy_st_));
...
InfoToken *InfoTokenVector::GetRawPointer() const { return h_data_; }
